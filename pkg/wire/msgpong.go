...
// MsgPong implements the Message interface and represents a bitcoin pong message which is used primarily to confirm
// that a connection is still valid in response to a bitcoin ping message (MsgPing). This message was not added until
// protocol versions AFTER BIP0031Version.
type MsgPong struct {
...
func (msg *MsgPong) BtcDecode(
	...
	if pver <= BIP0031Version {
		str := fmt.Sprintf(
			"pong message invalid for protocol "+
				"version %d", pver,
		)
...
func (msg *MsgPong) MaxPayloadLength(pver uint32) uint32 {
	plen := uint32(0)
	// The pong message did not exist for BIP0031Version and earlier. NOTE: > is not a mistake here. The BIP0031 was
	// defined as AFTER the version unlike most others.
	if pver > BIP0031Version {
		// Nonce 8 bytes.
		plen += 8
	}
