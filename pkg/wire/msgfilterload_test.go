...
func TestFilterLoadLatest(t *testing.T) {
	...
	if cmd := msg.Command(); cmd != wantCmd {
		t.Errorf(
			"NewMsgFilterLoad: wrong command - got %v want %v",
			cmd, wantCmd,
		)
	}
	...
	if e != nil {
		t.Errorf("encode of MsgFilterLoad failed %v e <%v>", msg, e)
	}
	// Test decode with latest protocol version.
	readmsg := MsgFilterLoad{}
	e = readmsg.BtcDecode(&buf, pver, enc)
	if e != nil {
		t.Errorf("decode of MsgFilterLoad failed [%v] e <%v>", buf, e)
	}
}
...
func TestFilterLoadCrossProtocol(t *testing.T) {
	...
	if e != nil {
		t.Errorf(
			"encode of NewMsgFilterLoad failed %v e <%v>", msg,
			e,
		)
	}
	...
	if e == nil {
		t.Errorf(
			"decode of MsgFilterLoad succeeded when it shouldn't have %v",
			msg,
		)
	}
}
...
func TestFilterLoadMaxFilterSize(t *testing.T) {
	...
	if e == nil {
		t.Errorf(
			"encode of MsgFilterLoad succeeded when it shouldn't "+
				"have %v", msg,
		)
	}
	...
	if e == nil {
		t.Errorf(
			"decode of MsgFilterLoad succeeded when it shouldn't "+
				"have %v", msg,
		)
	}
}
...
func TestFilterLoadMaxHashFuncsSize(t *testing.T) {
	...
	if e == nil {
		t.Errorf(
			"encode of MsgFilterLoad succeeded when it shouldn't have %v",
			msg,
		)
	}
	newBuf := []byte{
...
func TestFilterLoadWireErrors(t *testing.T) {
	...
	baseFilterLoadEncoded = append(
		baseFilterLoadEncoded,
