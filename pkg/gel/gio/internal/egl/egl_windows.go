...
func loadDLLs() error {
	...
	return loadDLL(
		syscall.NewLazyDLL("d3dcompiler_47.dll"),
		"d3dcompiler_47.dll",
	)
}
...
func eglChooseConfig(disp _EGLDisplay, attribs []_EGLint) (_EGLConfig, bool) {
	...
	var ncfg _EGLint
	a := &attribs[0]
	r, _, _ := _eglChooseConfig.Call(
		uintptr(disp), uintptr(unsafe.Pointer(a)),
...
func eglCreateContext(
	disp _EGLDisplay, cfg _EGLConfig, shareCtx _EGLContext,
	attribs []_EGLint,
) _EGLContext {
	a := &attribs[0]
	c, _, _ := _eglCreateContext.Call(
		uintptr(disp), uintptr(cfg),
...
func eglCreateWindowSurface(
	disp _EGLDisplay, cfg _EGLConfig,
	win NativeWindowType, attribs []_EGLint,
) _EGLSurface {
	a := &attribs[0]
	s, _, _ := _eglCreateWindowSurface.Call(
		uintptr(disp), uintptr(cfg),
...
func eglGetConfigAttrib(
	disp _EGLDisplay, cfg _EGLConfig, attr _EGLint,
) (_EGLint, bool) {
	var val uintptr
	r, _, _ := _eglGetConfigAttrib.Call(
		uintptr(disp), uintptr(cfg),
...
func eglInitialize(disp _EGLDisplay) (_EGLint, _EGLint, bool) {
	var maj, min uintptr
	r, _, _ := _eglInitialize.Call(
		uintptr(disp), uintptr(unsafe.Pointer(&maj)),
...
func eglMakeCurrent(
	...
	r, _, _ := _eglMakeCurrent.Call(
		uintptr(disp), uintptr(draw), uintptr(read),
		uintptr(ctx),
	)
	return r != 0
}
