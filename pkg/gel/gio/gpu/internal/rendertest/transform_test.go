...
func TestClipScale(t *testing.T) {
	run(
		t, func(o *op.Ops) {
			a := f32.Affine2D{}.Scale(f32.Point{}, f32.Pt(2, 2)).Offset(
				f32.Pt(
					10,
					10,
				),
			...
			paint.FillShape(
				o,
				red,
				clip.Rect(image.Rect(0, 0, 1000, 1000)).Op(),
			)
		}, func(r result) {
...
func TestRotateTexture(t *testing.T) {
	run(
		t, func(o *op.Ops) {
			...
			squares.Add(o)
			a := f32.Affine2D{}.Offset(f32.Pt(30, 30)).Rotate(
				f32.Pt(40, 40),
				math.Pi/4,
			)
			op.Affine(a).Add(o)
...
func TestComplicatedTransform(t *testing.T) {
	run(
		t, func(o *op.Ops) {
			...
			clip.RRect{
				Rect: f32.Rect(0, 0, 100, 100), SE: 50, SW: 50, NW: 50,
				NE: 50,
			}.Add(o)
...
func TestTransformOrder(t *testing.T) {
	...
	run(
		t, func(o *op.Ops) {
			...
			c := f32.Affine2D{}.Offset(f32.Pt(-10, -10)).Scale(
				f32.Point{},
				f32.Pt(0.5, 0.5),
			)
			op.Affine(c).Add(o)
