...
func NewConverter(
	workDir WorkDir, packageName, shadersDir string,
	directCompute bool,
) *Converter {
...
func (conv *Converter) Run(out io.Writer) error {
	...
	if len(shaders) == 0 || err != nil {
		return fmt.Errorf(
			"failed to list shaders in %q: %w", conv.shadersDir,
			err,
		)
	}
...
func (conv *Converter) Shader(shaderPath string) (
	[]driver.ShaderSources, error,
) {
	type Variant struct {
	...
	if err != nil {
		return nil, fmt.Errorf(
			"failed to parse template %q: %w", shaderPath,
			err,
		)
	}
	...
	for i, variantArg := range variantArgs {
		...
		if err != nil {
			return nil, fmt.Errorf(
				"failed to execute template %q with %#v: %w",
		...
		sources.GLSL100ES, _, _ = conv.ShaderVariant(
			shaderPath, variantName,
			buf.Bytes(), "es", "100",
		)

		var metadata Metadata
		sources.GLSL300ES, metadata, err = conv.ShaderVariant(
			shaderPath,
		...
		sources.GLSL130, _, err = conv.ShaderVariant(
			shaderPath, variantName,
			buf.Bytes(), "glsl", "130",
		)
		if err != nil {
		...
		hlsl, _, err := conv.ShaderVariant(
			shaderPath, variantName, buf.Bytes(),
			"hlsl", "40",
		)
		if err != nil {
			return nil, fmt.Errorf("failed to convert HLSL:\n%w", err)
		}
		sources.HLSL, err = conv.fxc.Compile(
			shaderPath, variantName,
		...
		sources.GLSL150, _, err = conv.ShaderVariant(
			shaderPath, variantName,
			buf.Bytes(), "glsl", "150",
		)
		if err != nil {
...
func (conv *Converter) ShaderVariant(
	shaderPath, variant string, src []byte,
	lang, profile string,
) (string, Metadata, error) {
	spirv, err := conv.glslvalidator.Convert(
		shaderPath, variant,
		lang == "hlsl", src,
	)
	if err != nil {
		return "", Metadata{}, fmt.Errorf(
			"failed to generate SPIR-V for %q: %w",
			shaderPath, err,
		)
	}
	...
	if err != nil {
		return "", Metadata{}, fmt.Errorf(
			"failed to extract metadata for shader %q: %w",
			shaderPath, err,
		)
	}
...
func (conv *Converter) ComputeShader(shaderPath string) (
	[]driver.ShaderSources,
	error,
) {
	...
	if err != nil {
		return nil, fmt.Errorf(
			"failed to convert compute shader %q: %w",
			shaderPath, err,
		)
	}
	...
	sources.GLSL310ES, err = conv.spirv.Convert(
		shaderPath, "", spirv, "es",
		"310",
	)
	if err != nil {
		return nil, fmt.Errorf(
			"failed to convert es compute shader %q: %w",
			shaderPath, err,
		)
	}
	...
	if err != nil {
		return nil, fmt.Errorf(
			"failed to convert hlsl compute shader %q: %w",
			shaderPath, err,
		)
	}

	dxil, err := conv.fxc.Compile(
		shaderPath, "0", []byte(hlslSource), "main",
		"5_0",
	)
	if err != nil {
		return nil, fmt.Errorf(
			"failed to compile hlsl compute shader %q: %w",
			shaderPath, err,
		)
	}
