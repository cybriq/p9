...
func (w *Window) Fill(
	col string, dxn l.Direction, radius float32, corners int,
	embed l.Widget,
) *Filler {
	return &Filler{
		Window: w, col: col, w: embed, dxn: dxn,
...
func (f *Filler) Fn(gtx l.Context) l.Dimensions {
	...
	fill(
		gtx, f.Colors.GetNRGBAFromName(f.col), dL[0].Size, f.cornerRadius,
		f.corners,
	)
...
func fill(
	gtx l.Context, col color.NRGBA, bounds image.Point, radius float32,
	cnrs int,
) {
	rect := f32.Rectangle{
