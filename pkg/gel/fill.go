...
func (w *Window) Fill(col string, dxn l.Direction, radius float32, corners int,
	embed l.Widget,
) *Filler {
...
func (f *Filler) Fn(gtx l.Context) l.Dimensions {
	...
	dL := GetDimensionList(gtx1, 1,
		func(gtx l.Context, index int) l.Dimensions {
			return f.w(gtx)
		},
	)
	fill(gtx, f.Colors.GetNRGBAFromName(f.col), dL[0].Size, f.cornerRadius,
		f.corners,
	)
