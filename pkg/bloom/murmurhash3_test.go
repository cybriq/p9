...
func TestMurmurHash3(t *testing.T) {
	var tests = []struct {
		...
		{
			0x00000000, []byte{0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66},
			0xb074502c,
		},
		{
			0x00000000, []byte{0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77},
			0x8034d2a0,
		},
		{
			0x00000000,
			[]byte{0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88},
			0xb4698def,
		},
	}
	for i, test := range tests {
		...
		if result != test.out {
			t.Errorf(
				"MurmurHash3 test #%d failed: got %v want %v\n",
				i, result, test.out,
			)
			continue
		}
