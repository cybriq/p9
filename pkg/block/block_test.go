...
func TestBlock(t *testing.T) {
	...
	if msgBlock := b.WireBlock(); !reflect.DeepEqual(msgBlock, &Block100000) {
		t.Errorf(
			"Block: mismatched Block - got %v, want %v",
	...
	if gotHeight := b.Height(); gotHeight != wantHeight {
		t.Errorf(
			"Height: mismatched height - got %v, want %v",
	...
	for i := 0; i < 2; i++ {
		...
		if !hash.IsEqual(wantHash) {
			t.Errorf(
				"Hash #%d mismatched hash - got %v, want %v",
				i, hash, wantHash,
			)
		}
	}
	...
	for i := 0; i < 2; i++ {
		...
		if len(transactions) != len(wantTxHashes) {
			t.Errorf(
				"Transactions #%d mismatched number of "+
		...
		for j, tx := range transactions {
			...
			if !hash.IsEqual(wantHash) {
				t.Errorf(
					"Transactions #%d mismatched hashes "+
	...
	for i := 0; i < 2; i++ {
		...
		if !bytes.Equal(serializedBytes, block100000Bytes) {
			t.Errorf(
				"Hash #%d wrong bytes - got %v, want %v", i,
	...
	if !reflect.DeepEqual(txLocs, wantTxLocs) {
		t.Errorf(
			"TxLoc: mismatched transaction location information "+
...
func TestBlockErrors(t *testing.T) {
	...
	if testErr.Error() != wantErr {
		t.Errorf(
			"OutOfRangeError: wrong error - got %v, want %v",
	...
	if e != io.EOF {
		t.Errorf(
			"NewFromBytes: did not get expected error - "+
	...
	if e != io.EOF {
		t.Errorf(
			"TxLoc: did not get expected error - "+
				"got %v, want %v", e, io.EOF,
		)
	}
}
