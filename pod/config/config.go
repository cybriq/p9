...
func (c *Config) Initialize(hf func(ifc interface{}) error) (e error) {
	...
	for i := range options {
		...
		if options[i].Name() == "datadir" {
			...
			if _, e = options[i].ReadInput(optVals[i]); !E.Chk(e) {
				...
				c.ConfigFile.Def = filepath.Join(
					datadir,
	...
	for i := range options {
		if options[i].Name() == "network" {
			...
			c.WalletFile.Def = filepath.Join(
				datadir, optVals[i],
				constant.DbName,
			)
	...
	if configPath != "" {
		...
		if datadir != "" {
			...
			if resolvedConfigPath, e = filepath.Abs(
				filepath.Clean(
					filepath.Join(
						datadir,
						constant.PodConfigFilename,
...
func (c *Config) processCommandlineArgs(args []string) (
	...
	var commandsStart, commandsEnd = -1, -1
	var found, helpFound bool
	for i := range args {
		T.Ln(
			"checking for commands:", args[i], commandsStart, commandsEnd,
			"current arg index:", i,
		)
		var depth, dist int
		if found, depth, dist, cm, e = c.Commands.Find(
			args[i], depth, dist,
			false,
		); E.Chk(e) {
			continue
		...
		if found {
			...
			if oc, ok := commands[depth]; ok {
				if !helpFound {
					e = fmt.Errorf(
						"second command found at same depth '%s' and '%s'",
						oc.Name, cm.Name,
					)
					return
				} else {
					...
					if cmdd, ok := commands[depth]; ok {
						I.Ln(
							"base level command is already occupied by",
							cmdd.Name+"; marking end of commands",
						)
						// commandsEnd--
						T.Ln(
							"commandStart", commandsStart, commandsEnd,
			...
			T.Ln(
				"found command", cm.Name, "argument number", i, "at depth",
				depth, "distance", dist,
			)
			commands[depth] = *cm
		} else {
			...
			T.Ln(
				"argument", args[i], "is not a command", commandsStart,
				commandsEnd,
			)
	...
	if len(commands) == 0 {
		...
		if len(cmds) > 0 {
			...
			if cmds[0] != 1 {
				e = fmt.Errorf(
					"commands must include base level item for disambiguation %v",
					cms,
				)
			}
			prev := cmds[0]
			for i := range cmds {
				...
				if !found {
					e = fmt.Errorf(
						"multiple commands are not a path on the command tree %v",
						cms,
					)
					return
				}
	...
	if commandsStart > 0 {
		...
		for i := range args {
			...
			if o, val, e = c.GetOption(args[i]); E.Chk(e) {
				e = fmt.Errorf(
					"argument %d: '%s' lacks a valid opt prefix", i,
					args[i],
				)
				return
			}
	...
	T.Ln(
		"options to pass to child processes to match config:",
		args[:commandsStart],
	)
	...
	c.ForEach(
		func(ifc opt.Option) bool {
			...
			for i := range allNames {
				helpCommand.Commands = append(
					helpCommand.Commands, cmds.Command{
						Name:  allNames[i],
						Title: dt.desc,
