...
func GetConfigs() (c config.Configs) {
	...
	c = config.Configs{
		...
		"WalletFile": text.New(
			...
			filepath.Join(
				string(datadir.Load().([]byte)), "mainnet",
				constant.DbName,
			),
		),
		"WalletOff": binary.New(
			meta.Data{
				Aliases:       []string{"WO"},
				Group:         "debug",
				Tags:          tags("wallet"),
